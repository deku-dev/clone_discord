...
export const changeServer = (server: string) => async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
  await dispatch(updateActiveUserList(server.split('-')[1]));
...
export const loadUserData = (userId: string) => async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
  ...
  if (res.data.servers) {
    ...
    await dispatch(updateActiveUserList(Object.keys(res.data.servers)[0].split('-')[1]));
    await dispatch<LoadUserDataAction>({ type: ACTION.GET_INITIAL_DATA, payload: res.data });
  }
};
