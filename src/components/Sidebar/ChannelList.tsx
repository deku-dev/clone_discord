...
import {MoreVert, Person, Settings} from '@material-ui/icons';
import {
  Avatar,
  Button,
  IconButton,
...
export default function ChannelList(props: ChannelListProps) {
  ...
  useEffect(() => {
    if (voiceJoinUserId !== '') {
      ...
      gotMessageFromServer(userId, rtcSignalData);
    }
  ...
  const onUserJoin = (newUserId: string, clients: { userName: string; userId: string }[]) => {
    ...
    clients.forEach(user => {
      // If new client isn't in our list
    ...
    // Don't create offer to ourselves on first join
